...
public class VoronoiClassic {
   ...
   "vec3 voronoi(float scaleFactor){" +
    	...
    	"if(mod(evalSquare.x, 2.) == 1. && mod(evalSquare.y, 2.) == 1.){minDist = min(minDist, 0.5);}" +
    		...
    		"featurePoint1 = (currentSquare + elem1)/scaleFactor + clamp(vec2(sin(jitterSpeed*elem1.x*timeF), sin(jitterSpeed*elem1.y*timeF))/(5.*scaleFactor*elem1), vec2(0.), vec2(1.4/scaleFactor));" +
    		"featurePoint2 = (currentSquare + elem2)/scaleFactor + clamp(vec2(sin(jitterSpeed*elem2.x*timeF), sin(jitterSpeed*elem2.y*timeF))/(5.*scaleFactor*elem2), vec2(0.), vec2(1.4/scaleFactor));" +
   		...
   		"minDist = min(minDist, minDistPre);" +
    		...
    		"if(currentCoords == featurePoint1){" +
    		"color = texture2D(lookupTexture, currentCoords).rgb;" +
    		"} else{" +
    		"color = texture2D(lookupTexture, currentCoords).rgb;" +
    		"}" +
    ...
    "return 1.8*color*(1.-minDist*scaleFactor);" +
    		"}" +
